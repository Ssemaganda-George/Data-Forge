...
export function IntegrationsManager() {
  ...
  async function saveGitHub() {
    ...
    try {
      ...
      const data = (await res.json()) as { error?: string; username?: string };
      if (!res.ok) throw new Error(data.error ?? "Save failed");
      setGithubConnected(data.username ?? githubUsername);
      setGithubToken("");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Save failed");
    } finally {
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Push exports as release assets. Create a fine-grained or classic PAT with{" "}
              <code className="text-[11px] bg-gray-100 px-1 rounded">repo</code> scope.
            </p>
          </div>
          {githubConnected && (
            <span className="text-xs font-medium text-green-700 bg-green-50 px-2 py-1 rounded">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block">
            ...
            <input
              ...
              placeholder={githubConnected ?? "github-username"}
              className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
            />
          </label>
          <label className="block">
            <span className="text-xs font-medium text-gray-600">Personal access token</span>
            <input
              type="password"
              value={githubToken}
        ...
        <div className="mt-4 flex gap-2">
          <Button
            ...
            onClick={() => saveGitHub()}
          >
            {githubConnected ? "Update token" : "Connect GitHub"}
          </Button>
          {githubConnected && (
